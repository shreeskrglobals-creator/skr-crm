...
st.markdown('<div class="subtitle">Tailored for Nagpur, India (IST) | Real-time Country Peak Activity Tracker</div>', unsafe_allow_html=True)
...
# 🔐 FIXED: Hardcoded India Standard Time (IST) Clock Engine (+5:30 UTC Offset)
ist_offset = timezone(timedelta(hours=5, minutes=30))
now_nd = datetime.now(ist_offset)
current_time_ist = now_nd.time()
current_day_str = now_nd.strftime("%A")
today_str = now_nd.strftime('%Y-%m-%d')
...
# Sidebar UI (Forced India Nagpur Time Layout)
...
with tab2:
    ...
    with st.form("new_lead_form", clear_on_submit=True):
        ...
        with col2:
            ...
            next_follow = st.date_input("Scheduled Follow-up Action Date", now_nd.date() + timedelta(days=2))
            ...
            with st.expander(header_title):
                ...
                with col_b:
                    ...
                    try:
                        ...
                        parsed_date = now_nd.date()
                    updated_date = st.date_input("Reschedule Date", parsed_date, key=f"date_mod_{idx}")
                
                custom_notes = str(row['What Customer Said']) if pd.notna(row['What Customer Said']) else ""
                updated_feed = st.text_area("Log Fresh Response Text Here:", value=custom_notes, key=f"feed_mod_{idx}")
