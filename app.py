...
# 1. Timezone Setup using Python's native built-in zones (No extra packages needed!)
# India Standard Time (IST) is UTC + 5.5 hours
    ...
    if df.empty:
        ...
        for idx, row in df.iterrows():
            with st.expander(f"💼 {row['Company Name']} ({row['Country']}) — Status: {row['Status']}"):
                ...
                with col_b:
                    current_status_index = ["New / No Contact", "Awaiting Reply", "Active Negotiation", "Converted"].index(row['Status'])
                    new_status = st.selectbox("Update Status", ["New / No Contact", "Awaiting Reply", "Active Negotiation", "Converted"], key=f"status_{idx}", index=current_status_index)
